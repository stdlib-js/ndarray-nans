...
tape( 'the function returns a NaN-filled array (dtype=float16, order=row-major)', function test( t ) {
	var expected;
	var opts;
	var arr;

	expected = new Float16Array( [ NaN, NaN, NaN, NaN ] );
	...
	t.strictEqual( String( getDType( arr ) ), opts.dtype, 'returns expected value' );
	t.deepEqual( getShape( arr ), [ 2, 2 ], 'returns expected value' );
	t.strictEqual( isSameFloat16Array( getData( arr ), expected ), true, 'returns expected value' );
	t.strictEqual( getOrder( arr ), opts.order, 'returns expected value' );

	t.end();
});

tape( 'the function returns a NaN-filled array (dtype=float16, order=column-major)', function test( t ) {
	var expected;
	var opts;
	var arr;
